        ...
        player?.addPeriodicTimeObserver(forInterval: CMTimeMake(value: 1, timescale: 100), queue: DispatchQueue.main) {
            ...
            if let time = Float(timeString) {
                if self?.currentItem != nil {
                    if self?.currentItem?.asset != nil {
                        self?.playerObserverDelegate?.didTrack(progress: time, total: Float(CMTimeGetSeconds((self?.currentItem?.asset.duration)!)))
                    }
                }
...
extension IGPlayerView: PlayerControls {
    ...
    func handleSeek(seekTime:Int){
        let time = CMTimeMake(value: Int64(seekTime), timescale: 1)
