...
class DPArrowMenuView: UIControl {
  ...
  lazy var menuTableView: UITableView = {
    let chatTableView = UITableView(frame: CGRect.zero, style: UITableView.Style.plain)
    chatTableView.backgroundColor = UIColor.clear
    chatTableView.delegate = self
    chatTableView.dataSource = self
    chatTableView.separatorColor = configuration.menuSeparatorColor
    chatTableView.layer.cornerRadius = configuration.cornerRadius
    chatTableView.clipsToBounds = true
    chatTableView.registerCell([DPPopOverMenuCell.self])
...
extension DPArrowMenuView: UITableViewDelegate {
  ...
  func tableView(_ chatTableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
    return configuration.menuRowHeight
  }

  func tableView(_ chatTableView: UITableView, didSelectRowAt indexPath: IndexPath) {
