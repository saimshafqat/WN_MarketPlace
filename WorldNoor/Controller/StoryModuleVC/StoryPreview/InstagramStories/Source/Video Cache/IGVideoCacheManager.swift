...
class IGVideoCacheManager {
    ...
    private lazy var mainDirectoryUrl: URL? = {
        let documentsUrl = self.fileManager.urls(for: FileManager.SearchPathDirectory.documentDirectory, in: FileManager.SearchPathDomainMask.userDomainMask).first
        return documentsUrl
    }()
        ...
        do {
            ...
            if let string = urlString, let url = URL(string: string) {
                do {
                    ...
                    LogClass.debugLog("Unable to remove the item: \(error)")
                }
            }else {
    ...
    private func directoryFor(stringUrl: String) -> URL? {
        if stringUrl.contains("https") || stringUrl.contains("http") {
            ...
            if stringUrl != "" {
                let fileName = URL(string: stringUrl)?.lastPathComponent
                let fileUrl = FileBasedManager.shared.getFileURLIfExist(fileName: fileName ?? "")
                return fileUrl
            }
        }
