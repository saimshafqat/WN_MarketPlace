...
class NewPageModel: NSObject , PickerviewDelegate {
    ...
    var parentView : NewPageDetailVC!
    var arrayBottom = [[String : String]]()
    var selectedTab = selectedUserTab.nothing
    var indexImage = 1
    
    var imageType = ""
    var isCoverPhoto = 0
    var categoryLikePageItem: CategoryLikePageModel?
    var parent: NewPageDetailVC?
...
extension NewPageModel:UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        if section == 0 {
            ...
            if self.parentView.groupObj!.isAdmin && self.selectedTab == .timeline{
                return 4
            }
            return 3
        }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 0 {
            ...
            } else if indexPath.row == 3 && self.parentView.groupObj!.isAdmin {
...
//                    SharedManager.shared.hideLoadingHubFromKeyWindow()
    ...
    func ProfileImageHeaderCell(tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cellImageHEader = tableView.dequeueReusableCell(withIdentifier: "NewPageImageCell", for: indexPath) as! NewPageImageCell
        
        cellImageHEader.groupObj = self.parentView.groupObj
        cellImageHEader.reloadData()
        
        cellImageHEader.delegate = parent
    ...
    func ProfileUserNameCell(tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cellUserName = tableView.dequeueReusableCell(withIdentifier: "PageNameVC", for: indexPath) as! PageNameVC
        
        cellUserName.groupObj = self.parentView.groupObj
        cellUserName.reloadName()
        if let categoryLikePageItem {
            cellUserName.updateCatePageLike(categoryLikePageItem)
        }
    ...
    func NoRecordFound(tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cellUserName.lblInfo.text = "No page found".localized()
    ...
    func ProfileUserTabCell(tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        guard let cellUserTab = tableView.dequeueReusableCell(withIdentifier: "ProfileUserTabCell", for: indexPath) as? ProfileUserTabCell else {
            return UITableViewCell()
        }
        ...
        cellUserTab.lblNews.text = "Reviews"
    ...
    func ProfileUserinfoCell(tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cellUserTab = tableView.dequeueReusableCell(withIdentifier: "NewPageaboutCell", for: indexPath) as! NewPageaboutCell
...
//            SharedManager.shared.hideLoadingHubFromKeyWindow()
    ...
    func callingFeedService(lastPostID:String, isLastTrue:Bool, isRefresh:Bool , isReview : Bool = false){
        ...
        if self.parentView.groupObj?.groupID.count == 0 {
            return
        }
        let userToken = SharedManager.shared.userToken()
        self.showloadingBar?()
...
//        SharedManager.shared.showOnWindow()
        Loader.startLoading()
        var parameters = ["action": "page/newsfeed","page_id":self.parentView.groupObj!.groupID, "token": SharedManager.shared.userToken()]
...
//            SharedManager.shared.hideLoadingHubFromKeyWindow()
    ...
    func callingImageService(lastPostID:String, isLastTrue:Bool, isRefresh:Bool){
        ...
        var parameters = ["action": "page/photos/more","page_id":self.parentView.groupObj!.groupID, "token": SharedManager.shared.userToken()]
    ...
    func callingVideoService(lastPostID:String, isLastTrue:Bool, isRefresh:Bool){
        ...
        var parameters = ["action": "page/videos/more","page_id":self.parentView.groupObj!.groupID, "token": SharedManager.shared.userToken()]
        if isLastTrue {
            parameters["starting_point_id"] = lastPostID
        }
        ...
        if let isFeedData = feedObj.data {
            if isFeedData.count == 0 {
                ...
                for indexObj in 0..<newArray.count {
                    ...
                    if indexObj % 10 == 0 {
                        if (self.parentView as? NewPageDetailVC) != nil  {
...
extension NewPageModel : ProfileTabSelectionDelegate {
...
extension NewPageModel:UITextViewDelegate {
...
extension NewPageModel : ProfileImageSelectionDelegate {
...
extension NewPageModel:UIScrollViewDelegate {
...
extension NewPageModel : CellDelegate {
