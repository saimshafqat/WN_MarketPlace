    ...
    public init?(audioAssetURL: URL) {
        ...
        do {
            ...
            guard let assetTrack = audioAsset.tracks(withMediaType: .audio).first else {
                LogClass.debugLog("ERROR loading asset track")
                return nil
            }
            ...
            LogClass.debugLog("ERROR loading asset \(error)")
            return nil
        }
    }
        ...
        assetReader.asset.loadValuesAsynchronously(forKeys: ["duration"]) {
            ...
            case .loaded:
                ...
                DispatchQueue.global(qos: qos).async {
                    ...
                    default:
                        LogClass.debugLog("ERROR: reading waveform audio data has failed \(self.assetReader.status)")
                        completionHandler(nil)
                    }
                }

            case .failed, .cancelled, .loading, .unknown:
                LogClass.debugLog("failed to load due to: \(error?.localizedDescription ?? "unknown error")")
                completionHandler(nil)
            @unknown default:
                LogClass.debugLog("failed to load due to: \(error?.localizedDescription ?? "unknown error")")
                completionHandler(nil)
