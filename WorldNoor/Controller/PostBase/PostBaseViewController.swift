...
class PostBaseViewController: PaginatedViewController {
    
    // MARK: - Lazy Properties -
    lazy var viewModel: PostBaseViewModel = {
    ...
    override func viewDidLoad() {
        ...
        //        self.navigationItem.title = "watch".localized()
        veiwModelCallBacks()
        addObserver()
    }
    ...
    // MARK: - Methods -
    func setupViewModel() -> PostBaseViewModel? {
        return nil
    }
    ...
    func veiwModelCallBacks() {
        ...
        viewModel.moreClosure = {  [weak self] postObj, indexPath in
            guard let self else { return }
            // let postMore = PostMenuVC.instantiate(fromAppStoryboard: .StoryModule)
            // self?.presentVC(postMore)
            // strognSelf.openBottomSheet(postMore, sheetSize: [.fullScreen])
...
extension PostBaseViewController: DismissReportSheetDelegate {
...
extension PostBaseViewController: DismissReportDetailSheetDelegate {
