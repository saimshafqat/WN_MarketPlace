...
public class CircularProgressBar: CALayer {
    ...
    public init(radius: CGFloat, position: CGPoint, innerTrackColor: UIColor, outerTrackColor: UIColor, lineWidth: CGFloat, showOnPost: Bool = true) {
        super.init()
        ...
        let size = CGSize(width: radius * 2, height: radius)
        ...
        progressLabel.center.y = showOnPost ? position.y - 10 : position.y
        progressLabel.font = UIFont.boldSystemFont(ofSize: showOnPost ? radius * 0.27 : radius * 0.5)
        progressLabel.text = "0%"
        progressLabel.textColor = showOnPost ? .white : innerTrackColor
        ...
        completedLabel.text = "Completed".localized()
        if showOnPost {
