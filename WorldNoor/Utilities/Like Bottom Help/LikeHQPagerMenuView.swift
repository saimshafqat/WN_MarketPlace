...
public struct LikeHQPagerMenuViewItemProvider {
...
public protocol LikeHQPagerMenuViewDataSource: class {
    func numberOfItems(inPagerMenu menuView: LikeHQPagerMenuView)-> Int
    func pagerMenuView(_ menuView: LikeHQPagerMenuView, itemAt index: Int)-> LikeHQPagerMenuViewItemProvider?
}

public protocol LikeHQPagerMenuViewDelegate: class {
    func menuView(_ menuView: LikeHQPagerMenuView, didChangeToIndex index: Int)
}

open class LikeHQPagerMenuView: UIView {
    ...
    fileprivate var highlightView = LikeHQRoundCornerView(frame: CGRect.zero)
    
    fileprivate var buttons: [UIButton] = []
    
    fileprivate var labels: [UILabel] = []
    
    open weak var dataSource: LikeHQPagerMenuViewDataSource? {
        didSet {
            reloadData()
        }
    }
    
    open weak var delegate: LikeHQPagerMenuViewDelegate?
...
//            highlightView.backgroundColor = item.selectedBackgroundColor
    ...
    private func createButton(atIndex index: Int)-> UIButton {
        ...
        button.addTarget(self, action: #selector(LikeHQPagerMenuView.buttonPressed(button:)), for: .touchUpInside)
        return button
    }
...
open class LikeHQRoundCornerView: UIView {
    open override func layoutSubviews() {
        super.layoutSubviews()
