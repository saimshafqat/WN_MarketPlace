...
private extension EZPlayerFairPlayResourceLoaderDelegate {
    ...
    func prepareAndSendContentKeyRequest(resourceLoadingRequest: AVAssetResourceLoadingRequest) {
        guard let url = resourceLoadingRequest.request.url, let assetIDString = url.host else {
            LogClass.debugLog("Failed to get url or assetIDString for the request object of the resource.")
            return
        }

        func __finishLoading(code: Int, errorDescription: String)  {
            LogClass.debugLog(errorDescription)
        ...
        if let filePathURLForPersistedContentKey = filePathURLForPersistedContentKey() {
            ...
            if FileManager.default.fileExists(atPath: filePathURLForPersistedContentKey.path) {
                ...
                do {
                    ...
                    LogClass.debugLog("Error initializing Data from contents of URL: \(error.localizedDescription)")
                    resourceLoadingRequest.finishLoading(with: error)
                    return
                }
            }
        ...
        do {
            ...
            LogClass.debugLog("Error obtaining key request data: \(error.domain) reason: \(error.localizedFailureReason ?? "")")
            resourceLoadingRequest.finishLoading(with: error)
            return
        }
        ...
        if shouldPersist, #available(iOS 9.0, *){
            ...
            do{
                ...
                LogClass.debugLog("Error creating persistent content key: \(error)")
                resourceLoadingRequest.finishLoading(with: error)
                return
            }
            ...
            do {
                ...
                if persistentContentKeyURL == documentURL {
                    LogClass.debugLog("failed to create the URL for writing the persistent content key")
                ...
                do {
                    ...
                    guard let dataRequest = resourceLoadingRequest.dataRequest else {
                        LogClass.debugLog("no data is being requested in loadingRequest")
                    ...
                    LogClass.debugLog("failed writing persisting key to path: \(persistentContentKeyURL) with error: \(error)")
                    resourceLoadingRequest.finishLoading(with: error)
                    return
                }
