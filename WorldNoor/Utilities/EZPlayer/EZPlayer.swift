...
open class EZPlayer: NSObject {
    ...
    open var isHideSound: Bool = false
    open var isShowCotrol : Bool = false
    
//    open var isShowSound: Bool = false
    ...
    var callbackPH: ((_ indexPAth: IndexPath , _ urlVideo : String) -> Void)?
    ...
    open fileprivate(set) var state = EZPlayerState.unknown{
        didSet{
            LogClass.debugLog("old state: \(oldValue)")
            LogClass.debugLog("new state: \(state)")

            
            if state == .readyToPlay {
                self.callbackPH!(self.indexPath ?? IndexPath.init(row: 0, section: 0) , self.contentURL?.absoluteString ?? "")
            }
            if oldValue != state{
                ...
                case  .readyToPlay,.playing ,.pause,.seekingForward,.seekingBackward,.stopped,.bufferFinished://cactus todo
                    ...
                    if state == .stopped {
                        ...
                        if let viewcontrol = (self.controlView as? EZPlayerControlView) {
//                            viewcontrol.btnReplay.isHidden = false
                            
                            
                            self.player!.seek(to: .zero)
                            self.player!.play()
                        }
                        if let viewcontrol = (self.controlViewForEmbedded as? EZPlayerControlView) {
//                            viewcontrol.btnReplay.isHidden = false
                            self.player!.seek(to: .zero)
                            self.player!.play()
                        }
...
//                    03214916649
    ...
    // new added
    open weak var scrollCollectionView: UICollectionView? {
        willSet{
            if scrollCollectionView != newValue {
                if let view = scrollCollectionView {
                    view.removeObserver(self, forKeyPath: #keyPath(UICollectionView.contentOffset))
        ...
        didSet {
            if playerItem != oldValue {
                if let view = scrollCollectionView {
                    view.addObserver(self, forKeyPath: #keyPath(UICollectionView.contentOffset), options: NSKeyValueObservingOptions.new, context: nil)
                }
            }
        }
    }
    ...
    open func toEmbedded(animated: Bool = true , completion: ((Bool) -> Swift.Void)? = nil){
        ...
        func __endToEmbedded(finished :Bool)  {
            ...
            LogClass.debugLog("indexPath ===>__endToEmbedded")
            LogClass.debugLog(indexPath)
            
            self.videoGravity = .aspectFill
            self.setControlsHidden(false, animated: true)
...
//                        self.view.center = center
    ...
    open func setControlsHidden(_ hidden: Bool, animated: Bool = false){
        ...
        LogClass.debugLog("setControlsHidden ==> 1")
        LogClass.debugLog(self.isShowCotrol)
        
        
        if self.isShowCotrol {
        self.controlsHidden = true
            (self.controlView as? EZPlayerDelegate)?.player(self, playerControlsHiddenDidChange: true ,animated: animated )
            self.delegate?.player(self, playerControlsHiddenDidChange: true,animated: animated)
            NotificationCenter.default.post(name: .EZPlayerControlsHiddenDidChange, object: self, userInfo: [Notification.Key.EZPlayerControlsHiddenDidChangeKey: true,Notification.Key.EZPlayerControlsHiddenDidChangeByAnimatedKey: animated])
        }else {
    ...
    open  func updateCustomView(toDisplayMode: EZPlayerDisplayMode? = nil){
        ...
        LogClass.debugLog("indexPath ===>updateCustomView")
        LogClass.debugLog(indexPath)
...
//        NotificationCenter.default.addObserver(self, selector: #selector(self.applicationWillEnterForeground(_:)), name: UIApplication.willEnterForegroundNotification, object: nil)
    ...
    open func checkSound(boovalue : Bool = false ){
        if let viewcontrol = (self.controlViewForEmbedded as? EZPlayerControlView) {
            self.isHideSound = boovalue
            viewcontrol.fullEmbeddedScreenButton.isSelected = boovalue
            
            
            
//            viewcontrol.fullEmbeddedScreenButton.isHidden = false
//            viewcontrol.fullEmbeddedScreenButtonPressed(viewcontrol.fullEmbeddedScreenButton)
        }
    }
...
extension EZPlayer {
    override open func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        if let item = object as? AVPlayerItem, let keyPath = keyPath {
            if item == self.playerItem {
                ...
                case #keyPath(AVPlayerItem.playbackBufferEmpty):
                    ...
                    LogClass.debugLog("AVPlayerItem.playbackBufferEmpty ")
                case #keyPath(AVPlayerItem.playbackLikelyToKeepUp):
                    //2）然后是kAVPlayerItemPlaybackLikelyToKeepUp，从0变到1，说明可以播放了，这时候会自动开始播放

                    LogClass.debugLog("AVPlayerItem's playbackLikelyToKeepUp is changed")
                default:
                    break
                }
            }
        ...
        } else if let view = object as? UICollectionView ,let keyPath = keyPath {
            switch keyPath {
            case #keyPath(UICollectionView.contentOffset):
                if isChangingDisplayMode == true {
                    return
                }
                if view == self.scrollCollectionView {
                    if let indexPath = self.indexPath {
                        let cellRectInCollectionView = view.layoutAttributesForItem(at: indexPath)?.frame
                        
                        if cellRectInCollectionView != nil {
                            let cellRectInCollectionViewSuperView = view.convert(cellRectInCollectionView!, to: view.superview)
                            
                            if cellRectInCollectionViewSuperView.origin.y + cellRectInCollectionViewSuperView.size.height/2 < view.frame.origin.y + view.contentInset.top || cellRectInCollectionViewSuperView.origin.y + cellRectInCollectionViewSuperView.size.height/2 > view.frame.origin.y + view.frame.size.height - view.contentInset.bottom {
                                self.toFloat()
                            } else {
                                if let cell = view.cellForItem(at: indexPath) {
                                    if !cell.contentView.subviews.contains(self.view) {
                                        self.view.removeFromSuperview()
                                        self.embeddedContentView = self.embeddedContentView ?? cell.contentView
                                        self.embeddedContentView!.addSubview(self.view)
...
//                self.view.removeFromSuperview()
    ...
    public func pictureInPictureController(_ pictureInPictureController: AVPictureInPictureController, restoreUserInterfaceForPictureInPictureStopWithCompletionHandler completionHandler: @escaping (Bool) -> Void) {
        LogClass.debugLog("pip 关闭画中画且恢复播放界面")
