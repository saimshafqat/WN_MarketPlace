...
- (void)commitChanges:(void (^)(id))handler {
	...
	if (self.window == nil) {
        ...
        NSMutableIndexSet *deletedSectionChanges = [self.deletedSectionIndexes copy];
        NSMutableIndexSet *insertedSectionChanges = [self.insertedSectionIndexes copy];
        NSMutableArray *deletedItemIndexPathsChanges = [self.deletedItemIndexPaths copy];
        NSMutableArray *insertedItemIndexPathsChanges = [self.insertedItemIndexPaths copy];
        NSMutableArray *updatedItemIndexPathsChanges = [self.updatedItemIndexPaths copy];
        
        [self clearChanges];
        
		
		[self performBatchUpdates:^{
			[self deleteSections:deletedSectionChanges];
			[self insertSections:insertedSectionChanges];
			
			[self deleteItemsAtIndexPaths:deletedItemIndexPathsChanges];
			[self insertItemsAtIndexPaths:insertedItemIndexPathsChanges];
			[self reloadItemsAtIndexPaths:updatedItemIndexPathsChanges];
			
		} completion:^(BOOL finished) {
			[self clearChanges];
            handler([NSNumber numberWithBool:finished]);
		}];
