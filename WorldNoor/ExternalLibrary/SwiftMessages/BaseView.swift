...
open class BaseView: UIView, BackgroundViewable, MarginAdjustable {
    ...
    open func installBackgroundVerticalView(_ backgroundView: UIView, insets: UIEdgeInsets = UIEdgeInsets.zero) {
        ...
        regularWidthLayoutConstraints = [
            backgroundView.leftAnchor.constraint(greaterThanOrEqualTo: layoutMarginsGuide.leftAnchor,
                                                 constant: insets.left).with(priority: UILayoutPriority(rawValue: 900)),
            backgroundView.rightAnchor.constraint(lessThanOrEqualTo: layoutMarginsGuide.rightAnchor,
                                                  constant: -insets.right).with(priority: UILayoutPriority(rawValue: 900)),
            backgroundView.widthAnchor.constraint(lessThanOrEqualToConstant: 500).with(priority: UILayoutPriority(rawValue: 950)),
            backgroundView.widthAnchor.constraint(equalToConstant: 500).with(priority:
                                                                                UILayoutPriority(rawValue: 200)),
        ]
        installTapRecognizer()
    }
    ...
    public var layoutMarginAdditions: UIEdgeInsets {
        get {
            return UIEdgeInsets(top: topLayoutMarginAddition,
                                left: leftLayoutMarginAddition,
                                bottom: bottomLayoutMarginAddition,
                                right: rightLayoutMarginAddition)
        }
        set {
    ...
    private func updateBackgroundHeightConstraint() {
        if let existing = backgroundHeightConstraint {
            let view = existing.firstItem as? UIView ?? UIView()
    ...
    */
    ...
    open override func updateConstraints() {
        ...
        let onConstraints: [NSLayoutConstraint]
        let offConstraints: [NSLayoutConstraint]
        switch traitCollection.horizontalSizeClass {
        case .regular:
            onConstraints = regularWidthLayoutConstraints
            offConstraints = layoutConstraints
        default:
            onConstraints = layoutConstraints
            offConstraints = regularWidthLayoutConstraints
        }
        onConstraints.forEach { $0.isActive = true }
        offConstraints.forEach { $0.isActive = false }
    }
